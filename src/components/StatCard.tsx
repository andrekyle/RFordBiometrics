  ...
  return (
    <div className="rounded-2xl border bg-card p-3 sm:p-5 transition-all hover:shadow-lg azure-shadow">
      <div className="flex items-center justify-between mb-2 sm:mb-3">
        <p className="text-[10px] sm:text-xs font-light text-muted-foreground uppercase tracking-wide">{title}</p>
        <Icon className={cn(
      ...
      {trend && <p className="text-[10px] sm:text-xs font-light text-muted-foreground">{trend}</p>}
    </div>
  );
}
